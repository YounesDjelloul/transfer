<script setup lang="ts">
  ...
  const instanceDetails  = await getInstanceDetails(props.endpointUrl, props.instancePk)
  ...
  const onUpdate = handleSubmit(async (values, actions) => {
    ...
    try {
      ...
      const response = await updateInstanceDetailsRequest(props.endpointUrl, props.instancePk, values, props.updateAllowedMethod)
      fieldSelect.handleUpdateOptionCreationAffect(response.data, props.instancePk, props.fieldId)
...
<template>
  <VModal
    ...
    actions="right"
    noscroll
    @close="fieldSelect.toggleUpdatePopup"
  >
    <template #content>
    ...
    <template #action>
      <VButton :loading="isLoading" type="submit" @click="onUpdate" color="primary" raised>Create</VButton>
    </template>
  </VModal>
</template>
